...
class Handler(webapp2.RequestHandler):
    ...
    def initialize(self, *a, **kw):
        ...
        #
        # env.filters['logio'] = Logio
        # custom_text="aab amy custom filter works!"
        # result = env.get_template("myTemplate.html").render(data=custom_text)
        # print(result)

        if self.user:
            self.logio = "<div><a href='/blog/logout'>Logout</a></div>"
        else:
            self.logio = """<div><a href='/blog/login'>Login</a><br>
                            <a href='/blog/signup'>Signup</a></div>"""
...
class Welcome(Handler):
    ...
    def get(self):
        if self.user:
            ...
            # env.globals['user_id'] = user_id
            # template = env.get_template('welcome.html')
            # print template.render(user_id = str(self.user.key().id()))
            self.render(
                'welcome.html',
...
class Blog(Handler):
    ...
    def get(self):
        ...
        self.render("blog.html", articles=articles, logio=self.logio)
...
class NewPost(Handler):
    ...
    def get(self):
        if self.user:
            self.render("newpost.html", logio=self.logio)
        else:
...
class NewComment(Handler):
    ...
    def get(self, article_id):
        ...
        a = db.get(key)
        if self.user:
            self.render("newcomment.html", logio=self.logio)
        else:
            self.redirect("/blog/login")
